...
categorical_columns = ["segment", "role", 'context']#  "availableMethods" - одна из фичей, которую нужно добавить
...
def elbow_method(transformed_data, is_plot_needed=False)->int:
    # Метод локтя.
    inertia = []
    range_clusters = range(1, 11)  # Тестируем от 1 до 10 кластеров
    ...
    # Автоподбор оптимального числа кластеров
...
def get_cluster_summary(df, n_clusters):
    # Рассчитываем медианы и моды для каждого кластера
    cluster_summary = {}
...
def join_predicted_tree(df, transformed_features):
    ...
    X = transformed_features#.drop(["predicted_cluster", "clientId", "organizationId"], axis=1)
    y = df["predicted_cluster"]
    tree = DecisionTreeClassifier(max_depth=3, random_state=42)
    tree.fit(X, y)
    df['predicted_cluster_tree'] = tree.predict(transformed_features)  # Добавляем кластер в DataFrame
...
"""
def inference_kmeans_(model, client_id:str, ):
    user_prediction = answer.loc[index_element_for_predict]
    rules = explain_decision(model2, features.iloc[index_element_for_predict].values, features.columns)

    user_info = answer.iloc[index_element_for_predict]
    conditions, predicted_class,_ =  explain_decision(model2, features.iloc[index_element_for_predict].values, features.columns)
    print(conditions, predicted_class)

    print(f"\n\n\nМы рекомендуем вам **class #{predicted_class} - у него **описание плюсов**.\n Вы получили эту рекомендацию, так у таких же как вы пользователей:\n",
        get_interpretesions(conditions, user_info)
    )"""